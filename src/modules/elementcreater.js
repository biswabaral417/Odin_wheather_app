const elemcreater = ({ prop, parentId, text }) => {
  ...
  prop.match(/href=\s*'([^']*)'/) &&
    element.setAttribute("href", prop.match(/href=\s*'([^']*)'/)[1]); // add href if exists
  ...
  prop.match(/placeholder='[^']*'/) &&
    element.setAttribute("placeholder", prop.match(/placeholder='([^']*)'/)[1]);

  prop.match(/value='[^']*'/) &&
    element.setAttribute("value", prop.match(/value='([^']*)'/)[1]);

  prop.match(/type='[^']*'/) &&
    element.setAttribute("type", prop.match(/type='([^']*)'/)[1]);
  return element;
};
