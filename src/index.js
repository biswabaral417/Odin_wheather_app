  ...
  for (let i = 0; i < 24; i++) {
    ...
    if (day1[j] !== undefined) {
      ...
      document.getElementById(`temp${i}`).textContent =
        `${day1[j].temp_c}  \u00B0 c`;
      document.getElementById(`weatherInfo-icon_hour${i}`).src =
        day1[j].condition.icon;
    } else {
      document.getElementById(`time${i}`).textContent =
        j - 24 > 12 ? `${j - 24 - 12} pm` : `${j - 24} am`;
      document.getElementById(`temp${i}`).textContent = `${
        day2[j - 24].temp_c
      }  \u00B0 c`;
      document.getElementById(`weatherInfo-icon_hour${i}`).src =
        day2[j - 24].condition.icon;
    }
  }
  wheatherData.forecast.forecastday.forEach((element, i) => {
    if (compare(element.date)) {
      document.getElementById(`weekday${i - 1}`).textContent = new Date(
        element.date,
      )
        .toString()
        .split(" ")[0];
      document.getElementById(`weekdaytemp${i - 1}`).textContent =
        `${element.day.avgtemp_c} \u00B0 c`;
      document.getElementById(`weatherInfo-icon_week${i - 1}`).src =
...
const hoursbtn = elemcreater({
  prop: "button#showhoursbtn",
  parentId: "bottom",
  text: "hourly",
});
const weekssbtn = elemcreater({
...
const searchsvgpath = document.createElementNS(
